   ...
   message Person {
    ...
    function _G.test_io.test()
        ...
        fail(
            "-not-exists-",
            function()
                assert(pbio.read "-not-exists-")
            end
        )

        local chunk = protoc.new():compile(pbio.read "address.proto", "address.proto")
        ...
        fail(
            "string expected for field 'name', got boolean",
            function()
                pb.encode("Person", {name = true})
            end
        )

        fail(
            "type mismatch for field 'name' at offset 2, bytes expected for type string, got varint",
            function()
                pb.decode("Person", "\8\1")
            end
        )

        fail(
            "invalid varint value at offset 2",
            function()
                pb.decode("Person", "\16\255")
            end
        )
        ...
        fail(
            ...
            function()
                pb.decode("Person", "\10\255")
            end
        )

        fail(
            "un-finished bytes (len 10 at offset 3)",
            function()
                pb.decode("Person", "\10\10")
            end
        )
...
function _G.test_default()
    ...
    table_eq(
        copy_no_meta(pb.defaults "TestDefault"),
        {
            defaulted_int = 777,
            defaulted_bool = true,
            defaulted_str = "foo",
...
function _G.test_enum()
    ...
    fail(
        "invalid varint value at offset 2",
        function()
            pb.decode("TestEnum", "\8\255")
        end
    )
    fail(
        "number/string expected at field 'color', got boolean",
        function()
            pb.encode("TestEnum", {color = true})
        end
    )
    fail(
        "can not encode unknown enum 'foo' at field 'color'",
        function()
            pb.encode("TestEnum", {color = "foo"})
        end
    )
...
function _G.test_packed()
   ...
   message TestPacked {
    ...
    fail(
        "table expected at field 'packs', got boolean",
        function()
            pb.encode("TestPacked", {packs = true})
        end
    )
    ...
    eq(pb.tohex(pb.encode("MessageB", {messageValue = {{}}})), "0A 00")
    eq(pb.tohex(pb.encode("MessageB", {messageValue = {{intValue = 1}}})), "0A 02 08 01")
    pb.clear "MessageA"
    pb.clear "MessageB"
    pb.option "auto_default_values"
    assert(pb.type ".google.protobuf.FileDescriptorSet")
...
function _G.test_map()
   ...
   message TestMap {
    ...
    fail(
        "string expected for field 'key', got number",
        function()
...
function _G.test_conv()
    ...
    fail(
        ...
        function()
            conv.encode_sint64(true)
        end
    )
    fail(
        "integer format error: '@xyz'",
        function()
            conv.encode_sint64("@xyz")
        end
    )
    assert(pb.type ".google.protobuf.FileDescriptorSet")
end
...
function _G.test_buffer()
    ...
    eq(b:tohex(-1), "01")
    b:pack("f", 0.125)
    eq(b:tohex(-4), "00 00 00 3E")
    b:pack("F", 0.125)
    eq(b:tohex(-8), "00 00 00 00 00 00 C0 3F")
    b:pack("i", 4294967295)
    eq(b:tohex(-10), "FF FF FF FF FF FF FF FF FF 01")
    b:pack("j", 4294967295)
    eq(b:tohex(-1), "01")
    b:pack("u", 4294967295)
    eq(b:tohex(-5), "FF FF FF FF 0F")
    b:pack("x", 4294967295)
    eq(b:tohex(-4), "FF FF FF FF")
    b:pack("y", 4294967295)
    eq(b:tohex(-4), "FF FF FF FF")
    if _VERSION == "Lua 5.3" then
        b:pack("I", 9223372036854775807)
        eq(b:tohex(-9), "FF FF FF FF FF FF FF FF 7F")
        b:pack("J", 9223372036854775807)
        eq(b:tohex(-10), "FE FF FF FF FF FF FF FF FF 01")
        b:pack("U", 9223372036854775807)
        eq(b:tohex(-9), "FF FF FF FF FF FF FF FF 7F")
        b:pack("X", 9223372036854775807)
        eq(b:tohex(-8), "FF FF FF FF FF FF FF 7F")
        b:pack("Y", 9223372036854775807)
        eq(b:tohex(-8), "FF FF FF FF FF FF FF 7F")
    else
        b:pack("I", "#9223372036854775807")
        eq(b:tohex(-9), "FF FF FF FF FF FF FF FF 7F")
    ...
    fail(
        "integer format error: 'foo'",
        function()
            pb.pack("i", "foo")
        end
    )
    fail(
        "number expected for type 'int32', got boolean",
        function()
            pb.pack("i", true)
        end
    )
...
function _G.test_slice()
    ...
    fail(
        "invalid varint value at offset 1",
        function()
            pb.unpack(("\255"):rep(10), "v")
        end
    )
    fail(
        "invalid varint value at offset 1",
        function()
            pb.unpack("\255\255\255", "v")
        end
    )
    fail(
        "invalid varint value at offset 1",
    ...
    fail(
        "invalid bytes value at offset 1",
        function()
            pb.unpack("\3\1\2", "s")
        end
    )
    fail(
        "invalid fixed32 value at offset 1",
        function()
            pb.unpack("\1\2\3", "d")
        end
    )
    fail(
        "invalid fixed64 value at offset 1",
        function()
            pb.unpack("\1\2\3", "q")
        end
    )
    fail(
        "invalid sub string at offset 1",
        function()
            pb.unpack("\3\1\2", "c", 5)
        end
    )
    fail(
        "invalid varint value at offset 1",
        function()
            pb.unpack("\255\255\255", "i")
        end
    )
    fail(
        "invalid fixed32 value at offset 1",
        function()
            pb.unpack("\255\255\255", "x")
        end
    )
    fail(
        "invalid fixed64 value at offset 1",
    ...
    fail(
        "string/buffer/slice expected, got boolean",
        function()
            pb.unpack(true, "v")
        end
    )
    fail(
        "bytes wireformat expected at offset 1",
        function()
            slice "\1":enter()
        end
    )
...
function _G.test_load()
    ...
    buf:pack(
        "v(v(vsv(vsvvvvv(vvvv)vv)vvv(vv)v(vv)))vv",
        ...
        1,
        v(100),
        0, -- unknown field options
        v(100),
        0, -- unknown field entry
        v(100),
        0, -- unknown type entry
        s(8), -- DescriptorProto.oneof_decl
        v(100),
        0, -- unknown oneof entry
        s(7), -- DescriptorProto.options
        v(100),
        0, -- unknown type options
        v(100),
        0 -- unknown file options
    )
    eq(pb.load(buf:result()), true)
    fail(
        "unknown type <unknown>",
        function()
            pb.encode("load_test", {test_unknown = 1})
        end
    )
    fail(
        "unknown type <unknown>",
        function()
            pb.decode("load_test", "\8\1")
        end
    )

    buf:reset()
    buf:pack("v(v(vsv(vsvvvv)))", s(1), s(4), s(1), "load_test", s(2), s(1), "test_unknown", v(3), 1, v(4), 1)
    eq(pb.load(buf:result()), true)
    fail(
        "type mismatch for field 'test_unknown' at offset 2, <unknown> expected for type <unknown>, got varint",
        function()
            pb.decode("load_test", "\8\1")
        end
    )
    fail(
        "type mismatch for field 'test_unknown' at offset 2, <unknown> expected for type <unknown>, got 64bit",
        function()
            pb.decode("load_test", "\9\1")
        end
    )
    fail(
        "type mismatch for field 'test_unknown' at offset 2, <unknown> expected for type <unknown>, got bytes",
        function()
            pb.decode("load_test", "\10\1")
        end
    )
    fail(
        "type mismatch for field 'test_unknown' at offset 2, <unknown> expected for type <unknown>, got gstart",
        function()
            pb.decode("load_test", "\11\1")
        end
    )
    fail(
        "type mismatch for field 'test_unknown' at offset 2, <unknown> expected for type <unknown>, got gend",
        function()
            pb.decode("load_test", "\12\1")
        end
    )
    fail(
        "type mismatch for field 'test_unknown' at offset 2, <unknown> expected for type <unknown>, got 32bit",
        function()
            pb.decode("load_test", "\13\1")
        end
    )

    buf:reset()
    buf:pack("v(v(vsv(vsvvvv)))", s(1), s(4), s(1), "load_test", s(2), s(1), "test_unknown", v(3), 2, v(4), 1)
    eq(pb.load(buf:result()), true)
    ...
    buf:reset()
    buf:pack("v(v(vsv(vsvvvv)))", s(1), s(4), s(1), "load_test", s(2), s(1), "test_unknown2", v(3), 1, v(4), 1)
    eq(pb.load(buf:result()), true)
    ...
    buf:reset()
    buf:pack("v(v(vsv(vvvv)))", s(1), s(4), s(1), "load_test", s(2), v(3), 1, v(4), 1)
    eq(pb.load(buf:result()), true)

    buf:reset()
    buf:pack("v(v(vsv(vvvvvv)))", s(1), s(4), s(1), "load_test", s(2), v(3), 1, v(4), 1, v(5), 11)
    eq(pb.load(buf:result()), true)

    buf:reset()
    buf:pack("v(v(vsv(vvvv)))", s(1), s(4), s(1), "load_test", s(6), v(3), 1, v(4), 1)
    eq(pb.load(buf:result()), true)
